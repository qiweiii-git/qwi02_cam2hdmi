...
int ov5640_read(XIicPs *IicInstance,u16 addr,u8 *read_buf)
{
	...
	buf[0] = (addr >> 8);
	buf[1] = addr;
	if(i2c_wrtie_bytes(IicInstance,0x3c,buf,2) != XST_SUCCESS)
   {
		return XST_FAILURE;
	}
   if(i2c_read_bytes(IicInstance,0x3c,read_buf,1) != XST_SUCCESS)
   {
      return XST_FAILURE;
	}
   return XST_SUCCESS;
...
int ov5640_write(XIicPs *IicInstance,u16 addr,u8 data)
{
	...
	buf[1] = addr;
	buf[2] = data;
	if(i2c_wrtie_bytes(IicInstance,0x3c,buf,3) != XST_SUCCESS)
   {
		return XST_FAILURE;
   }   
	return XST_SUCCESS;
...
int sensor_write_array(XIicPs *IicInstance, struct reginfo *regarray)
{
   int err = 0, cnt;
   int i = 0;
